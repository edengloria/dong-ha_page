...
test("photo links support normal clicks, new tabs, and no-JavaScript browsing", async ({ page, browser }) => {
  ...
  await expect(plain.locator("a.photo-print")).toHaveCount(24)
  await plain.getByRole("link", { name: "Next >", exact: true }).first().click()
  await expect(plain).toHaveURL(/\/gallery\/photos\/page\/2\//)
  await expect(plain.locator("a.photo-print").first()).toHaveAttribute("data-image-id", "25")
