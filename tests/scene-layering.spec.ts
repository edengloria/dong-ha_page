...
test("inner pages retain sidebar decorations above panels and home restores the rest", async ({ page }) => {
  ...
  await page.goto("/")
  const foregroundCount = await page.locator(".scene-foreground .scene-sprite:visible").count()
  ...
  for (const route of ["/publications", "/gallery/photos", "/gallery/vinyl"]) {
    ...
    await expect(page.locator(".scene-foreground .scene-sprite:visible")).toHaveCount(3)
    await expect(page.locator('.scene-foreground .scene-sprite:visible:not([data-anchor^="sidebar"])')).toHaveCount(0)
    await expect(page.locator(".scene-sprite:visible")).toHaveCount(71)
  }
  await page.locator('.directory a[href="/"]').click()
  await expect(page.locator(".scene-foreground .scene-sprite:visible")).toHaveCount(foregroundCount)
