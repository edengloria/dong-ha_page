...
function LPCard({ 
  ...
  // Handle mobile tap to toggle
  const handleMobileTap = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    if (!isMobile) return
    e.stopPropagation()
    
    if (isActiveOnMobile) {
      // Deactivate: stop preview
      stopPreview()
    } else {
      // Activate: start preview
      onMobileActivate()
      isActiveRef.current = true
      playPreview()
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Mobile tap hint - show on first few items when not active */}
...
export default function LPCollection() {
  ...
  return (
    <>
      ...
      {/* Now Playing Bar - rendered via portal */}
      ...
      {/* LP Detail Modal - rendered via portal to escape transform context */}
      {mounted && createPortal(
        <AnimatePresence>
          {selectedLP && (
            <motion.div
